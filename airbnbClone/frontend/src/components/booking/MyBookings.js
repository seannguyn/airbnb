...
class MyBookings extends Component {
  ...
  getBookings = (userID, bookingList) => {
    ...
    for (let i = 0; i < bookingList.length; i++) {
      if (userID === bookingList[i].booker) res.push(bookingList[i]);
    }
    this.setState({myBookings: res});
  };

  // separate future, past, current booking into different array and set state
  ...
  requireReview = async (pastStay) => {
    ...
    console.log("TEMP PAST STAY: ", tempPastStay);
  ...
  componentDidMount() {
    ...
    const res2 = axios.get('https://localhost:8000/booking/');
        ...
        <center>
          {!isEmpty(earliestBooking) ?
            <div>
              ...
              <Booking key={earliestBooking.id} booking={earliestBooking}/>
            </div>
            : null
          }
        </center>
        <center>
          {!isEmpty(pastStay[0]) ?
            <div key={pastStay[0].id}>
              ...
              <Booking key={pastStay[0].id} booking={pastStay[0]} requireReviewItem={requireReviewList[0]}/>
            </div>
            : null
          }
        </center>
              ...
              return (
                <div key={booking.id}>
                  <div style={{padding: '1rem'}}>
                    <center>
                      <Booking key={booking.id} booking={booking} earliestBooking={earliestBooking}/>
                    </center>
                  </div>
                </div>
              );
          ...
          {myBookings.length !== 0 ?
            myBookings.map((booking) => {
              return (
                <div key={booking.id} style={{padding: '1rem'}}>
                  <center>
                    <Booking key={booking.id} booking={booking} pastStay={pastStay} currentUser={currentUser}/>
                  </center>
                </div>
              );
            })
