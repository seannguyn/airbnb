...
class LoginDialog extends React.Component {
  ...
  errorCheck() {
    ...
    if (this.props.Signin_form === false && password !== password_confirm) {
      console.log("ENTER");
      this.setState({error: {password: 'password mismatch'}, error_bool: true});
      return true;
    }
  ...
  async handleLogin(dispatch, e) {
    ...
    if (this.errorCheck() === true) return;

    if (this.props.Signin_form === true) {
      const account = {
      ...
      await axios.post('/api-token-auth/', account)
        ...
        .catch(error => {
          ...
          this.setState({
            error: {username: "Check your username again", password: "Check your password again"},
            error_bool: true
          });
        });
      ...
      await axios.post('/rest-auth/registration/', newAccount)
        .catch(error => {
          ...
          if (error.response.data.username) {
            this.setState({
              error: {username: error.response.data.username}, error_bool: true});
          }
          if (error.response.data.password1) {
            this.setState({
              error: {password: "password needs more sophistication", username: this.state.error.username},
              error_bool: true
            });
          }
        });
        ...
        {value => {
          ...
          return (
            <Dialog open={dialog.open} aria-labelledby="form-dialog-title"
                    disableBackdropClick={backdrop} className={classes.layout}>

              <CloseIcon onClick={LoginDialog.closeDialog.bind(this, dispatch)} style={{float: 'right'}}/>
