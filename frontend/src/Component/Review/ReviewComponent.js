...
class ReviewComponent extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      currentPage: 1,
      perPage: 5,
      totalPage: Math.round(parseInt(props.review.length, 10) /  parseInt(5, 10)+0.5),
      filteredReview: [],
      filtering: false,
      searchKey: '',
      searchKeyFixed: '',
  ...
  handleSearch(e) {
    ...
    this.setState({
      ...
      currentPage: 1,
      totalPage: Math.round(parseInt(filteredReview.length, 10) /  parseInt(5, 10)+0.5),
  ...
  resetSearch() {
    this.setState({
      ...
      filteredReview: [],
      currentPage: 1,
      totalPage: Math.round(parseInt(this.props.review.length, 10) /  parseInt(5, 10)+0.5),
      searchKeyFixed: '',
      searchKey: ''
    })
  }
  ...
  render () {
    ...
    if (this.state.filtering === false) {
      ...
      review = currentReview.map((r) => {
        return(
          <div  key={uuid.v4()} style={{marginTop:'10px', marginBottom:'10px'}}>
            <ReviewRow review={r} />
            <Divider/>
            <Divider/>
          </div>
      ...
      if (this.state.filtering === true) {
        searchHeadline.push(<SearchHeadline resetSearch={this.resetSearch.bind(this)} guest={this.state.filteredReview.length} searchKey={this.state.searchKeyFixed} key={uuid.v4()}/>)
      }
      const indexOfLast = currentPage * perPage;
      const indexOfFirst = indexOfLast - perPage;
      const currentReview = this.state.filteredReview.slice(indexOfFirst, indexOfLast);
      ...
      <div>
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-6">
              <form noValidate autoComplete="off" onSubmit={this.handleSearch.bind(this)}>
                <TextField
                  label="Search"
                  type="search"
