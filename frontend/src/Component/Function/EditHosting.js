  ...
  onChange = e => {
    ...
    if (e.target.name === "date_end") {
      ...
      if (
        ...
        date_end < this.state.date_end_old
      ) {
        this.setState({ error_date: true, error: true })
      } else {
        this.setState({ error_date: false, error: false })
    ...
    if (newStartDate < this.state.date_start_old) {
      date_free_1 = date_free_1.concat(
        enumerateDaysBetweenDates(
          ...
          yyyymmdd(
            new Date(
              this.state.date_start_old.setDate(
                this.state.date_start_old.getDate() - 1
              )
            )
          )
        )
    ...
    if (newEndDate > this.state.date_end_old) {
      date_free_2 = date_free_2.concat(
        enumerateDaysBetweenDates(
          yyyymmdd(
            new Date(
              this.state.date_end_old.setDate(
                this.state.date_end_old.getDate() + 1
              )
            )
          ),
          date_end
