...
export default class SearchSection extends React.Component {
  handleChange = name => event => {
    this.setState({ [name]: event.target.value }, () => {
      if (
        this.state.price_lower.length > 0 &&
        this.state.price_upper.length > 0
      ) {
        if (
          parseInt(this.state.price_lower, 10) >
          parseInt(this.state.price_upper, 10)
        ) {
